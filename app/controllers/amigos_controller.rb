class AmigosController < ApplicationController
  ...
  # GET /amigos.json
  def index
    @amigos = Amigo.all.reverse_order
  end
  ...
  def create
    ...
    respond_to do |format|
      if @amigo.save
        format.html { redirect_to amigos_path, notice: 'Amigo was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @amigo.update(amigo_params)
        format.html { redirect_to amigos_path, notice: 'Amigo was successfully updated.' }
  ...
  private
    ...
    def amigo_params
      params.require(:amigo).permit(:nomeCompleto, :telefone)
    end
end
